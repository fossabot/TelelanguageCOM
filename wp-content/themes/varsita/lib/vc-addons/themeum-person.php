...
add_shortcode( 'themeum_person', function($atts, $content = null) {
	...
	$output  	.= '<figcaption>';
	if ($person_name) $output 	.= '<h3 class="person-title">' . $person_name . '</h3>';
	if ($person_deg) $output 	.= '<span class="person-deg">' . $person_deg . '</span>';
	if ($person_desc) $output 	.= '<p>' . $person_desc. '</p>';
	...
	$output 	.= '</figcaption>'; //person-details
	$output 	.= '</figure>'; //person-img
	

	$output .= '</div>'; //person-box
