...
if ($themeum_options['header-fixed']){
	...
	if(isset($headerbg)){
		$output .= '#masthead.sticky{ background-color: rgba('.hex2rgb($headerbg).',.95); }';
	}
	$output .= '#masthead.sticky{position:fixed; z-index:99999;margin:0 auto 30px; width:100%;box-shadow: 0 0 3px 0 rgba(0, 0, 0, 0.22);}';
	$output .= '#masthead.sticky #header-container{ padding:10px 0;transition: padding 200ms linear; -webkit-transition:padding 200ms linear;}';
	$output .= '#masthead.sticky .navbar.navbar-default{ background: rgba(255,255,255,.95); border-bottom:1px solid #f5f5f5}';
	$output .= '.page-template-homepage-transparent-php #masthead.sticky #main-menu .nav>li>a{ color:#444}';
}
...
if(isset($themeum_options['header-padding-bottom'])){
	$output .= '#header-container{ padding-bottom: '. (int) esc_attr($themeum_options['header-padding-bottom']) .'px; }';
}
...
if(isset($themeum_options['bottom-bg'])){
	$output .= '#bottom{ background: '.esc_attr($themeum_options['bottom-bg']) .'; }';
}
...
if(isset($themeum_options['certificate-bg-color'])){
	$output .= ".certificate{ background-color: ".$themeum_options['certificate-bg-color']."; }";
}
