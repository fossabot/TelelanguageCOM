...
if($portfolios->have_posts()):
	...
	while ($portfolios->have_posts()): $portfolios->the_post();
		...
		$output .= '<li class="themeum-post-item portfolio-item'.esc_html($term_name).'">';
		...
		{
			if ($folio_video) {
				$output .= '<a data-rel="prettyPhoto[pp_gal]" href="'.esc_url($folio_video).'"><i class="fa fa-search"></i></a>';
			}else if(has_post_thumbnail($post->ID)) {
				$large_image 	= wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'full');
				$output .= '<a data-rel="prettyPhoto[pp_gal]" href="'.esc_url($large_image[0]).'"><i class="fa fa-search"></i></a>';
			}else{
