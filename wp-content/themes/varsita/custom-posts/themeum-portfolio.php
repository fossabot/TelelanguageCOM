...
function themeum_portfolio_shortcode($atts, $content=null)
{
	...
	if (isset($themeum_options)) {
		$folio_per_page		= esc_attr($themeum_options['folio-per-page']);
		$disable_filter  	= esc_attr($themeum_options['disable-filter']);
		$disable_single  	= esc_attr($themeum_options['disable-single']);
		$disable_popup  	= esc_attr($themeum_options['disable-poup']);
		$disable_loadmore  	= esc_attr($themeum_options['disable-load-more']);
	}

	$filters = get_terms('portfolio-tag');
	$output = '';
	...
	if (!$disable_filter)
	{
		...
		{
			$output .= '<li><a class="btn btn-default" href="#" data-filter=".'.esc_attr($filter->slug).'">'.esc_html($filter->name).'</a></li>';
		}
	...
	$output .= '<ul id="themeum-area" class="portfolio-items col-'.esc_attr($column).'">';
	...
	foreach ($myportfolios as $post)
	{
		...
		{
			if ($folio_video) {
				$output .= '<a data-rel="prettyPhoto[pp_gal]" class="btn-preview" href="'.esc_url($folio_video).'"><i class="fa fa-search-plus"></i></a>';
			}else if(has_post_thumbnail($post->ID)) {
				$large_image 	= wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'full');
				$output .= '<a data-rel="prettyPhoto[pp_gal]" class="btn-preview" href="'.esc_url($large_image[0]).'"><i class="fa fa-search-plus"></i></a>';
			}else{
		...
		if($term_name != '')
        {
	        $output .= '<span class="portfolio-category">'.esc_html($term_name2).'</span>';
	    }
		$output .= '</div>';
		$output .= '</li>';
	...
	if (!$disable_loadmore)
	{
		if($allposts->publish > $folio_per_page){
			$output .= '<div class="clearfix load-wrap">';
			$output .= '<span class="ajax-loader">Ajax Loader</span>';
			$output .= '<div class="clearfix"></div>';
			$output .= '<a class="load-more btn btn-primary" id="post-loadmore" data-per_page="'.$folio_per_page.'" data-url="'.get_template_directory_uri().'/custom-posts/themeum-loadmore.php'.'" data-total_posts="'.$allposts->publish.'" data-col_grid="'.esc_attr($column).'" href="#">'.__('Load More', 'themeum').'</a>';
			$output .= '</div>';
		}
	}
